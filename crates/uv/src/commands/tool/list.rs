...
pub(crate) async fn list(
    ...
    let latest: FxHashMap<PackageName, Option<DistFilename>> = if outdated && !valid_tools.is_empty()
    {
        ...
        let reporter =
            LatestVersionReporter::from(printer).with_length(valid_tools.len() as u64);
